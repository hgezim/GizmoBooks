...
def urlencode_noplus(query):
    ...
    for i,pair in enumerate(query):
        if i > 0:
            retStr += "&"
        retStr += "%s=%s" % (urllib.quote(pair[0]), urllib.quote(pair[1]))
...
def buildQuery(argv):
    ...
    query_string = urlencode_noplus([(k, argv[k]) for (k) in sorted(argv.keys()) if argv[k]])
...
def buildRequest(argv):
    ...
    argv.update({'Service':'AWSECommerceService',
                 ...
                 #'Timestamp':"%s-%s-%sT%s:%s:%sZ" % (time.gmtime().tm_year, time.gmtime().tm_mon, time.gmtime().tm_mday, time.gmtime().tm_hour, time.gmtime().tm_min, time.gmtime().tm_sec),
