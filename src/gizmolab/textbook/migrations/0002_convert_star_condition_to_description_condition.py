...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Convert decimal point conditions to description conditions (Amazon like)"
        
        old_to_new_condition = {
                                5.0: 4.0, # used-like new
                                4.5: 4.0, 
                                4.0: 3.0, # used-very good
                                3.5: 2.0, # used-good
                                3.0: 2.0, 
                                2.5: 1.0, # used-acceptable
                                2.0: 1.0,
                                1.5: 1.0,
                                1.0: 1.0,
                                }

        for copy in orm.Book_Copy.objects.all():
            new_condition = old_to_new_condition[float(copy.condition)]
            copy.condition = Decimal(str(new_condition)) 
            copy.save()


    def backwards(self, orm):
        "Can't undo as mapping is ambigous."
        raise RuntimeError("Cannot convert description conditions to star conditions.")
