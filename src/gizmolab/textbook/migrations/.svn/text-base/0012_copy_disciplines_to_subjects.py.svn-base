...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Create Subject root nodes (copied from disciplines)."
                
        for i, book in enumerate(orm.Book.objects.all()):
            try:
                subject = orm.Subject.objects.get(name=book.discipline)
            except:
                subject = orm.Subject.objects.create(name=book.discipline,
                                                     lft=1,
                                                     rght=2,
                                                     tree_id=i+1,
                                                     level=0)
    ...
    def backwards(self, orm):
        "Copy the first subject into discipline."
        
        for book in orm.Book.objects.all():
            try:
                discipline = book.subjects.all()[0].name
            except:
                discipline = "Other"
            book.discipline = discipline
            book.save()
    ...
    models = {
        ...
        'textbook.book_copy': {
            ...
            'buying_options': ('django.db.models.fields.CharField', [], {'default': "'1'", 'max_length': '1'}),
