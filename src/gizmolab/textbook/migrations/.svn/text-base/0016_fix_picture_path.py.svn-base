...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Append 'books/' to picture path."""
        
        for book in orm.Book.objects.all():
            book.picture = "books/%s" % (book.picture,)
            book.save()

    def backwards(self, orm):
        """Removes 'books/' from picture path."""
        
        for book in orm.Book.objects.all():
            if book.picture.name[:6] == "books/":
                book.picture = book.picture.name[6:]
                book.save()
